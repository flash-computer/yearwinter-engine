...
YWE_err YWE_ErrorHandler(YWE_Engine *game, YWE_err code)
{
	if(YWER_ERROR(code))
	{
		fprintf(stderr, FATAL_ERR_MSG "Something broke. Shit is fucked%s%s" YWE_FM_RESET "\n", (code & 0x100)?" in SDL ― ":"", SDL_GetError());
	}
	YWE_ExitGame(game, 1);
	return code;
}
...
YWE_err YWE_ResizeEvent(YWE_Engine *game, SDL_WindowEvent *event)
{
	...
	if(SDL_SetWindowSize(game->window, min_magnitude * game->winprop.aspect_x, min_magnitude * game->winprop.aspect_y))
	{
		return YWER_ALL_CLEAR;
	}
...
#undef PASS_BACK_ERR
#undef PASS_BACK_ERRPTR
#undef PASS_BACK_ERRINT
